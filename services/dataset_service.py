...
class DatasetService:
    ...
    def __init__(self):
        ...
        if not os.listdir(self.data_dir):
            ...
            print(
                f"Found {len(os.listdir(self.data_dir))/2} datasets in {self.data_dir}"
            )
    ...
    def generate_builtin_datasets(self):
        ...
        try:
            ...
            self._save_dataset(iris_df, "iris", "target", "Iris flower dataset")
            ...
            try:
                ...
                housing_df = pd.DataFrame(
                    data=np.hstack([housing.data, housing.target.reshape(-1, 1)]),
                    columns=list(housing.feature_names) + ["MedHouseVal"],
                )
                self._save_dataset(
                    housing_df,
                    "california",
                ...
                print(f"Skipping California housing: {str(e)}")
            ...
            cancer_df["target"] = (
                cancer_df["target"].astype(int).map({0: "malignant", 1: "benign"})
            )
            self._save_dataset(
                cancer_df, "breast_cancer", "target", "Breast cancer Wisconsin dataset"
            )
            ...
            digits_df.columns = [f"pixel_{i}" for i in range(digits_df.shape[1])]
            digits_df["target"] = digits.target
            self._save_dataset(
                digits_df, "digits", "target", "Handwritten digits dataset (8x8 images)"
            )
    ...
    def _save_dataset(
        self, df: pd.DataFrame, name: str, target_col: str, description: str
    ):
        """Helper method to save dataset and metadata."""
        # Save dataset
        ...
        metadata_path = os.path.join(self.data_dir, f"{name}.json")
    ...
    def save_dataset(self, content: bytes, filename: str, target_column: str) -> str:
        ...
        # Save metadata
        metadata = {"original_filename": filename, "target_column": target_column}
