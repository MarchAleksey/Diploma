...
class TrainingService:
    ...
    def train_model(
        ...
        try:
            ...
            noise_generator = self.noise_service.create_noise_generator(noise_type, noise_params)
            ...
            # Start MLflow run
            self.mlflow_logger.start_run(run_name=experiment_name or f"{model_name}_{noise_type}")
            ...
            self.training_status[experiment_id]['message'] = 'Training model without noise'
            ...
            # Confusion matrix
            visualizations['confusion_matrix'] = self.mlflow_logger.log_confusion_matrix(
                y, model_noisy.predict(X)
            )
            ...
            visualizations['learning_curve'] = self.mlflow_logger.log_learning_curve(
                model_noisy.model, X, y
            )
            
            # Feature importance (for models that support it)
            if hasattr(model_noisy.model, 'feature_importances_'):
                visualizations['feature_importance'] = self.mlflow_logger.log_feature_importance(
                    model_noisy.model.feature_importances_, feature_names
                )
            ...
            # Log model
            self.mlflow_logger.log_model(model_noisy.model, model_name)
    ...
    def get_experiment_results(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Get dataset info
        try:
            dataset_info = self.dataset_service.get_dataset_info(experiment['dataset_id'])
        ...
        if experiment['noise_type'] == 'label_flip':
            noise_level = f"{experiment['noise_params'].get('flip_ratio', 0) * 100:.1f}%"
            ...
            noise_level = f"{experiment['noise_params'].get('missing_ratio', 0) * 100:.1f}%"
        else:
            noise_level = "Unknown"
        ...
        for viz_name, viz_path in experiment['visualizations'].items():
            # For simplicity, we'll just use placeholder images
            # In a real application, you would load the actual images from MLflow
            visualizations[viz_name] = f"/api/visualizations/{experiment_id}/{viz_name}"
        ...
        return {
            ...
            'date': datetime.fromisoformat(experiment.get('timestamp', '')).strftime('%Y-%m-%d %H:%M:%S'),
            'metrics': metrics,
