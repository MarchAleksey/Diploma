...
def main():
    ...
    page = st.sidebar.radio(
        "Go to",
        ["Home", "Dataset", "Model Selection", "Noise Configuration", "Training", "Results"]
    )
...
def show_home_page():
    ...
    try:
        ...
        st.warning("Could not fetch system statistics. Make sure the backend is running.")
...
def show_model_page():
    ...
    try:
        ...
        if model_type:
            ...
            if selected_model:
                # Get model details
                model_details = next((model for model in model_types[model_type] if model["name"] == selected_model), None)
                ...
                if model_details:
                    ...
                    for param in model_details["parameters"]:
                        ...
                        if param_type == "float":
                            ...
                            hyperparams[param_name] = st.checkbox(f"{param_name}", value=param_default)
                        elif param_type == "select":
                            hyperparams[param_name] = st.selectbox(
                                f"{param_name}",
                                options=param.get("options", []),
                                index=param.get("options", []).index(param_default) if param_default in param.get("options", []) else 0
                            )
        ...
        if selected_noise_type:
            ...
            for param in noise_details["parameters"]:
                ...
                if param_type == "float":
                    ...
                    noise_params[param_name] = st.checkbox(f"{param_name}", value=param_default)
                elif param_type == "select":
...
def show_training_page():
    ...
    with col2:
        cv_folds = st.slider("Cross-Validation Folds", min_value=2, max_value=10, value=5, step=1)
        experiment_name = st.text_input("Experiment Name", value=f"{st.session_state.model}_{st.session_state.noise_type}")
        ...
        try:
            ...
            with st.spinner("Training in progress..."):
                ...
                if response["success"]:
                    ...
                    st.success(f"Training started successfully! Experiment ID: {response['experiment_id']}")
                    ...
                    while True:
                        status = api_client.get_training_status(response["experiment_id"])
...
def show_results_page():
    ...
    try:
        experiment_results = api_client.get_experiment_results(st.session_state.experiment_id)
        ...
        with tab1:
            st.image(experiment_results["visualizations"]["confusion_matrix"], use_column_width=True)
        
        with tab2:
            st.image(experiment_results["visualizations"]["roc_curve"], use_column_width=True)
        
        with tab3:
            st.image(experiment_results["visualizations"]["learning_curve"], use_column_width=True)
        
        with tab4:
            st.image(experiment_results["visualizations"]["feature_importance"], use_column_width=True)
